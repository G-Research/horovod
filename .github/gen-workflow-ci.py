...
def main():
    ...
    def trigger_buildkite_job(id: str, name: str, needs: List[str], mode: str, fail_on_test_failure: bool) -> str:
        if 'init-workflow' not in needs:
            needs.insert(0, 'init-workflow')
        return (f'  {id}:\n'
                f'    name: "{name}"\n'
    ...
    with open(path.joinpath('workflows', 'ci.yaml').absolute(), 'wt') as w:
        ...
        workflow = workflow_header() + jobs(
            ...
            trigger_buildkite_job(id='buildkite', name='Build and Test GPU (on Builtkite)', needs=['build-and-test'], mode='GPU NON HEADS', fail_on_test_failure=True),
            trigger_buildkite_job(id='buildkite-heads', name='Build and Test GPU heads (on Builtkite)', needs=['build-and-test'], mode='GPU HEADS', fail_on_test_failure=False),
